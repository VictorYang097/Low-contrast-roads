"""
Training codes on the Massachusetts Roads dataset
"""
import torch
import os
import argparse
...
def train(train_loader, model, optimizer, scheduler, epoch):
    ...
    if not os.path.exists("run/Massachusetts/{}".format(opt.model_name)):
        os.makedirs("run/Massachusetts/{}".format(opt.model_name))
    file = open("run/Massachusetts/{}/train_log.txt".format(opt.model_name), "a")
    for i, sample in enumerate(tqdm(train_loader)):
        ...
        if (i+1) % 30 == 0 or (i+1) == train_batch: #origin: 10
    ...
    iou = evaluator.Intersection_over_Union()
    if not os.path.exists("run/Massachusetts/{}/ckpt/".format(opt.model_name)):
        os.makedirs("run/Massachusetts/{}/ckpt/".format(opt.model_name))
    save_path = 'run/Massachusetts/{}/ckpt/'.format(opt.model_name)
...
def valid(valid_loader, model, epoch, best):
    ...
    if not os.path.exists("run/Massachusetts/{}".format(opt.model_name)):
        os.makedirs("run/Massachusetts/{}".format(opt.model_name))
    file = open("run/Massachusetts/{}/valid_log.txt".format(opt.model_name), "a")
    for i, sample in enumerate(tqdm(valid_loader)):
       ...
       # ---- data prepare ----
        ...
        if (i+1) % 3 == 0 or (i+1) == valid_batch: #origin:8
            ACC = evaluator.Pixel_Accuracy()
            ACC_class = evaluator.Pixel_Accuracy_Class()
            mIOU = evaluator.Mean_Intersection_over_Union()
            IOU = evaluator.Intersection_over_Union()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no_improve', type=int,
                        default=10, help='After a few epochs, if IOU doesn\'t improve, stop training')
    ...
    # ---- build datasets ----
    train_ds = massDataSet(opt, base_dir='/home/jovyan/work/Massachusetts/', split='train')
    valid_ds = massDataSet(opt, base_dir='/home/jovyan/work/Massachusetts/', split='valid')
    ...
    # Continue training
    if opt.resume:
        checkpoint = torch.load('run/Massachusetts/{}/ckpt/{}.pth.tar'.format(opt.model_name, opt.model_name))
        checkpoint_best = torch.load('run/Massachusetts/{}/ckpt/{}_best.pth.tar'.format(opt.model_name, opt.model_name))
