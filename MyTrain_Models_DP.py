"""
Training codes on the DeepGlobe dataset
"""
import torch
import os
import argparse
...
def train(train_loader, model, optimizer, scheduler, epoch):
    ...
    if not os.path.exists("run/DeepGlobe/{}".format(opt.model_name)):
        os.makedirs("run/DeepGlobe/{}".format(opt.model_name))
    file = open("run/DeepGlobe/{}/train_log.txt".format(opt.model_name), "a")
    for i, sample in enumerate(tqdm(train_loader)):
        ...
        if (i+1) % 30 == 0 or (i+1) == train_batch:
    ...
    iou = evaluator.Intersection_over_Union()
    if not os.path.exists("run/DeepGlobe/{}/ckpt/".format(opt.model_name)):
        os.makedirs("run/DeepGlobe/{}/ckpt/".format(opt.model_name))
    save_path = 'run/DeepGlobe/{}/ckpt/'.format(opt.model_name)
...
def valid(valid_loader, model, epoch, best):
    ...
    if not os.path.exists("run/DeepGlobe/{}".format(opt.model_name)):
        os.makedirs("run/DeepGlobe/{}".format(opt.model_name))
    file = open("run/DeepGlobe/{}/valid_log.txt".format(opt.model_name), "a")
...
if __name__ == '__main__':
    ...
    # ---- build datasets ----
    train_ds = deepDataSet(opt, base_dir='/home/jovyan/work/DeepGlobe', split='train')
    valid_ds = deepDataSet(opt, base_dir='/home/jovyan/work/DeepGlobe', split='valid')
    ...
    # Continue training
    if opt.resume:
        checkpoint = torch.load('run/DeepGlobe/{}/ckpt/{}.pth.tar'.format(opt.model_name, opt.model_name))
        checkpoint_best = torch.load('run/DeepGlobe/{}/ckpt/{}_best.pth.tar'.format(opt.model_name, opt.model_name))
